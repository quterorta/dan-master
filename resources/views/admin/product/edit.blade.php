                ...
                <form action="{{ route('product.update', $product['id']) }}" method="POST" enctype="multipart/form-data">
                  ...
                  <div class="card-body">
                    <div class="form-group">
                      ...
                      <input value="{{ $product->title }}" name="title" type="text" class="form-control" id="exampleInputEmail1" placeholder="Введите название товара" required>
                    </div>
                    <div class="form-group">
                      <label for="exampleInputPassword1">Код товара</label>
                      <input value="{{ $product->article }}" name="article" type="text" class="form-control" id="exampleInputPassword1" placeholder="Артикул" required>
                    </div>
                    <div class="form-group">
                        <label for="exampleInputPassword1">Описание товара</label>
                        <textarea name="description" class="form-control" id="exampleInputPassword1" placeholder="Описание товара" value="">{{ $product->description }}</textarea>
                      </div>
                    <div class="form-group">
                      <label for="exampleInputFile">Изображение товара</label>
                      <div><img style="width:15vw; margin: 10px 0;" src="{{ Storage::url($product->image) }}" alt=""></div>
                      <div class="input-group">
                        <div class="custom-file">
                            <input name="image" type="file" class="custom-file-input" id="exampleInputFile" value="{{ $product['image'] }}">
                            <input name="hidden-image" type="text" value="{{ $product['image'] }}" hidden>
